...
export default function App(){
    ...
    // ---- controls as STRINGS so user can clear 0 ----
    ...
    const g1       = n(G1,0.001);
    const v0n      = n(v0,0);
    const angleN   = n(angle,0);
    const h0n      = n(h0,0);
    const tempN    = n(tempC,15);
    const pressN   = n(pressureHpa,1013.25);

    const rho=useMemo(()=>airDensityFromTP(tempN,pressN),[tempN,pressN]);
    const {data,range,maxH,flightTime}=useMemo(()=>simulate({v0:v0n,angleDeg:angleN,h0:h0n,massKg,G1:g1,diameterMm:diameter,rho,xMax}),[v0n,angleN,h0n,massKg,g1,diameter,rho,xMax]);
        ...
        <div style={{maxWidth:1200,margin:'0 auto',color:'var(--text)'}}>
            ...
            <div className="card" style={{marginTop:16}}>
                <div className="grid">
                    ...
                    <label>Коэфф. (G1)<input type="number" step="0.001" value={G1} onChange={e=>setG1(e.target.value)} /></label>
                    <label>Начальная скорость (м/с)<input type="number" step="1" value={v0} onChange={e=>setV0(e.target.value)} /></label>
                    <label>Угол запуска (°)<input type="number" step="1" value={angle} onChange={e=>setAngle(e.target.value)} /></label>
                    <label>Высота запуска (м)<input type="number" step="0.1" value={h0} onChange={e=>setH0(e.target.value)} /></label>
