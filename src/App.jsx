...
const heightAt = (data, X) => {
    ...
    let i=0; while(i+1<data.length && data[i+1].x < X) i++;
    const a=data[i], b=data[i+1]||a; if(!a||!b) return NaN;
    if (b.x===a.x) return a.y;
    const r=(X-a.x)/(b.x-a.x); return a.y + r*(b.y-a.y);
};
...
const sampleHeights=(data,ranges)=> {
    ...
    for(const R of ranges){
        ...
        const a=data[i], b=data[i+1]||a; if(!a||!b){ out.push({distance:R,height:NaN}); continue; }
        const r=b.x!==a.x?(R-a.x)/(b.x-a.x):0; out.push({distance:R,height:a.y+r*(b.y-a.y)});
    }
    return out;
};
...
export default function App(){
    ...
    const solveAngleForZero = (zeroR) => {
        let lo=-10, hi=20;
        const f=(ang)=>{ const {data}=simulate({v0:v0n,angleDeg:ang,h0:h0n,massKg,G1:g1,diameterMm:diameter,rho,xMax:Math.max(xMax,zeroR+50)}); return heightAt(data, zeroR); };
        let flo=f(lo), fhi=f(hi), expand=0;
        while((isNaN(flo)||isNaN(fhi)||flo*fhi>0) && expand<4){ lo-=10; hi+=10; flo=f(lo); fhi=f(hi); expand++; }
        if(isNaN(flo)||isNaN(fhi)||flo*fhi>0) return 0;
        let best=0;
        for(let it=0; it<30; it++){ const mid=(lo+hi)/2; const fm=f(mid); best=mid;
    ...
    const angleDeg=useMemo(()=>solveAngleForZero(zeroRange),[zeroRange, v0n, h0n, massKg, g1, diameter, rho, xMax]);

    const {data,range,maxH,flightTime}=useMemo(()=>simulate({v0:v0n,angleDeg,h0:h0n,massKg,G1:g1,diameterMm:diameter,rho,xMax}),[v0n,angleDeg,h0n,massKg,g1,diameter,rho,xMax]);
        ...
        <div style={{minHeight:'100vh',padding:16}}>
            <div style={{maxWidth:1200,margin:'0 auto',color:'var(--text)'}}>
                ...
                <div style={{display:'flex',gap:8,marginTop:10,flexWrap:'wrap'}}>
                    {!ent.sub_active && <button className="btn btn-primary" onClick={buySubscription}>Оформить подписку ({priceStars} ⭐ / мес)</button>}
                    <button className="btn" onClick={exportCorrections}>Скачать таблицу поправок с шагом в 50 метров</button>
                </div>

                <div className="card" style={{ marginTop: 16 }}>
                    <p><b>Подписка:</b> 5 дней бесплатно, затем {priceStars} ⭐ / мес. Автопродление. Отмена — в истории платежей Telegram.</p>
                    <p><b>Инструкция:</b> для расчёта баллистики любого патрона необходимо ввести всего 4 параметра вашей пули, которые указаны на упаковке или можно найти в интернете: Масса (г), Диаметр (мм), Баллистический коэффициент (G1), Начальная скорость (м/с).</p>
                    <p><b>Поддержка:</b> @proteano</p>
                </div>

                <div className="card" style={{marginTop:16}}>
                    <div className="grid">
                        <label>Масса (г)<input type="number" step="0.1" value={massGr} onChange={e=>setMassGr(e.target.value)} /></label>
                        <label>Диаметр (мм)<input type="number" step="0.01" value={diameterMm} onChange={e=>setDiameterMm(e.target.value)} /></label>
                        <label>Баллистический коэффициент (G1)<input type="number" step="0.001" value={G1} onChange={e=>setG1(e.target.value)} /></label>
                        <label>Начальная скорость (м/с)<input type="number" step="1" value={v0} onChange={e=>setV0(e.target.value)} /></label>
                        <label>Расстояние пристрелки (м)<input type="number" step="1" value={zeroRangeStr} onChange={e=>setZeroRangeStr(e.target.value)} /></label>
                        <label>Температура (°C)<input type="number" step="0.1" value={tempC} onChange={e=>setTempC(e.target.value)} /></label>
                        <label>Давление (гПа)<input type="number" step="0.1" value={pressureHpa} onChange={e=>setPressureHpa(e.target.value)} /></label>
