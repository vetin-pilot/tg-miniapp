...
export default function App(){
  ...
  useEffect(()=>{
    ...
    if(tg){ tg.ready(); tg.expand();
      const theme=()=>{const p=tg.themeParams||{};document.documentElement.style.setProperty('--bg',p.bg_color||'#f7f7f7');document.documentElement.style.setProperty('--text',p.text_color||'#111');};
      ...
      fetch('/api/entitlement?initData='+encodeURIComponent(initData)).then(r=>r.json()).then(setEnt).finally(()=>setLoadingEnt(false));
    } else {
  ...
  const trialLeft = useMemo(()=>{
    const s=ent.trial_left_s|0; const d=Math.max(0,Math.floor(s/86400)), h=Math.max(0,Math.floor((s%86400)/3600));
    return `${d} д ${h} ч`;
  },[ent.trial_left_s]);
  ...
  const buySubscription=async()=>{
    ...
    const j=await r.json(); if(!j.ok) return alert(j.error||'Не удалось создать подписку');
  ...
  return <div style={{minHeight:'100vh',padding:16}}>
    <div style={{maxWidth:1200,margin:'0 auto',color:'var(--text)'}}>
      <h1 style={{margin:0,fontSize:22,fontWeight:600}}>Траектория полёта — физическая модель{" "}
      ...
      <div className="card" style={{marginTop:16}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,marginTop:12,fontSize:14}}>
          <div><div>Дальность</div><b>{range.toFixed(1)} м</b></div>
          <div><div>Макс. высота</div><b>{(maxH*100).toFixed(0)} см</b></div>
          <div><div>Время полёта</div><b>{flightTime.toFixed(2)} с</b></div>
        </div>
        <div style={{marginTop:8,fontSize:14}}>Начальная энергия: <b>{E0J.toFixed(1)}</b> Дж (<b>{E0FPE.toFixed(1)}</b> фт·фунт)</div>
      </div>
