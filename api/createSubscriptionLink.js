...
export default async function handler(req,res){
  ...
  const body={ title:'Подписка PRO (мес)', description:'5 дней бесплатно, затем 300 ⭐/мес. Автопродление.',
    payload:`pro-month-${Date.now()}`, currency:'XTR', provider_token:'',
    prices:[{ label:'PRO месяц', amount: PRICE }], subscription_period: PERIOD };
  ...
  const j=await r.json(); if(!j.ok) return res.status(400).json(j); return res.status(200).json({ ok:true, link:j.result });
}
